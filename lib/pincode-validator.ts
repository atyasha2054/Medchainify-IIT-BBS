...
export function validatePincode(
  ...
  if (stateData) {
    ...
    const isInState = stateData.range.some(
      (r) => pinVal >= r.min && pinVal <= r.max,
    );

    if (!isInState) {
      // Get expected start digits for better error message
      const startsWith = stateData.range
        .map((r) => r.min.toString().substring(0, 2))
        .join(" or ");
      return {
        isValid: false,
    ...
    if (cityPrefixes) {
      const matchesCity = cityPrefixes.some((prefix) =>
